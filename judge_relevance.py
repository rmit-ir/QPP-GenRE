...
class Prompter:
    def __init__(self, args):
        prompts = {
            "binary": """Instruction: Please assess the relevance of the provided passage to the following question. 
            ...
            Output:""",
            "ikat": """You are a search quality rater evaluating the relevance of web pages.
            Given the persona of the user, user query, and a web page, you must provide a score on an integer scale of 0 to 4 to
            indicate to what extent the given document meets the information needs of the user.
...
def load_qpp_data(args):
        ...
        for qid, qtext in query_demon.items():
            ...
            pid_list = [
                ...
                for (pid, score) in sorted(
                    run_demon[qid].items(), key=lambda x: x[1], reverse=True
                )
            ]
            ...
            for pid in qrels_demon[qid]:
                ...
                passage_text = (
                    passage_dict["contents"]
                    if "contents" in passage_dict
                    else passage_dict["passage"]
                )
                passage_text = (
                ...
                demonstration += args.prompter.demonstration.format(
                    question=qtext, passage=passage_text, output=args.prompter.pos_label
                )
            ...
            demonstration += args.prompter.demonstration.format(
                question=qtext, passage=passage_text, output=args.prompter.neg_label
            )

            demonstration_list.append(demonstration)

        demonstration_list_sampled = random.sample(
            demonstration_list, args.num_demon_per_class
        )
    ...
    for qid, qtext in query.items():
        ...
        pid_list = [
            pid
            for (pid, score) in sorted(
                run[qid].items(), key=lambda x: x[1], reverse=True
            )
        ]
        ...
        if args.infer:
            for idx, pid in enumerate(pid_list[: args.k]):
                ...
                if args.query_demon_path is not None:
                    example["input"] = args.prompter.template.format(
            ...
            for pid in qrels[qid]:
                ...
                example["input"] = args.prompter.template.format(
                    demonstrations="", question=qtext, passage=passage_text
                )
                example["output"] = args.prompter.pos_label
                examples.append(example)
                pos_num += 1
            ...
            if len(pid_list) < args.num_negs:
                print(
                    f"Skip sampling negatives for {qid} because it has insufficient negatives:\n{qrels[qid]}, {run[qid]}, {pid_list_}"
                )
                continue
...
def train(args):
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        if not args.train_on_inputs:
            ...
            tokenized_prompt_label["labels"] = [
                IGNORE_INDEX
            ] * prompt_len + tokenized_prompt_label["labels"][
                prompt_len:
...
def infer(args):
    ...
    for start_idx in tqdm.tqdm(it):
        ...
        enc = tokenizer(
            [example["input"] for example in examples[rng]],
            padding=True,
            truncation=True,
            max_length=args.max_input_length,
            return_tensors="pt",
...
if __name__ == "__main__":
    ...
    args.query_type = "-".join(
        args.query_path.split("/")[-1].split(".")[1].split("-")[1:]
    )
    ...
    if not args.rj:
        args.retriever = "-".join(
            args.run_path.split("/")[-1].split(".")[1].split("-")[1:]
        )
    ...
    if args.infer is True:
        ...
        if args.checkpoint_name:
            ...
            if args.rj:
                if args.query_demon_path is not None:
                    dataset_name_demon = args.query_demon_path.split("/")[-1].split(
                        "."
                    )[0]
                    retriever_demon = "-".join(
                        args.run_demon_path.split("/")[-1].split(".")[1].split("-")[1:]
                    )
                    ...
                    args.setup = (
                        f"{args.qrels_name}.{args.query_type}-{args.base_model}"
                    )
            else:
                if args.query_demon_path is not None:
                    dataset_name_demon = args.query_demon_path.split("/")[-1].split(
